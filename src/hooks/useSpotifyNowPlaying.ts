...
export function useSpotifyNowPlaying(sessionId: string) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/spotify/now-playing?sid=${encodeURIComponent(sessionId)}`,
      );
      if (res.status === 401) {
      ...
      // Preload new artwork before updating state
