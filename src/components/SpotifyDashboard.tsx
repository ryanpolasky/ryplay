...
const PANELS = [
  ...
  { id: "unlock", label: "More" },
];

export default function SpotifyDashboard() {
  const { spotifySession, setSpotifySession } = useUser();
  ...
  const sessionId = spotifySession?.id ?? "";
  ...
  const { music, loading: musicLoading, error } = useSpotifyNowPlaying(sessionId);
  ...
  // Toast on track change when not on now-playing page
  useEffect(() => {
    ...
    <div className="h-dvh w-screen overflow-hidden bg-[#060609] text-white">
      ...
      >
        ...
        <div className="group/user relative">
          <span className="text-sm text-white/40 cursor-default flex items-center gap-1.5">
          ...
          <button
            onClick={() => setSpotifySession(null)}
      ...
      >
        {/* Panel 1: Now Playing + Recent Tracks */}
        <ErrorBoundary>
          <NowPlaying
            username={spotifySession.displayName}
            music={music}
            colors={colors}
        ...
        <ErrorBoundary>
          {shouldRender[3] ? (
            <UnlockPanel colors={colors} />
          ) : (
            <section id="unlock" className="min-h-dvh snap-start snap-always" />
          )}
        </ErrorBoundary>
      </main>
    </div>
