...
async function extractWithVibrant(img: HTMLImageElement): Promise<PaletteColors> {
  const mod = await vibrantPromise;
  // @ts-expect-error node-vibrant's exports vary across builds
  const Vibrant = mod.default ?? mod.Vibrant ?? mod;
  // Pass the already-loaded HTMLImageElement so Vibrant doesn't refetch.
  const p = await Vibrant.from(img).quality(5).getPalette();
...
export function useArtworkPalette(imageUrl: string | null) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const proxyUrl = artworkProxyUrl(imageUrl)!;

    // Single fetch for everything: load the image once via the shared
    // loader, then hand the resulting HTMLImageElement to Vibrant
    // (extractWithVibrant accepts an image directly, so it won't refetch).
    // VinylView's preload uses the same shared loader, and the proxy
    // serves `Cache-Control: immutable`, so any subsequent <img
    // src={proxyUrl}> in the tree hits the disk cache.
    loadArtworkImage(proxyUrl)
      .then(async (img) => {
        if (cancelled) return;
        try {
          const colors = await extractWithVibrant(img);
          if (cancelled) return;
          setPalette(colors);
          setIsExtracted(true);
        } catch {
          if (cancelled) return;
          // Vibrant failed — canvas fallback with the already-loaded image
          try {
